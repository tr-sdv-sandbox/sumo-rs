...
enum Command {
    ...
    /// Generate a single-use vHSM v3 ENROLL bootstrap token for one guest.
    ///
    /// Writes the raw 32-byte token (for the guest's firmware bank) and a
    /// `bootstrap.yaml` fragment containing only its SHA-256 hash (for the
    /// daemon-side state file). The daemon never sees the raw bytes — only
    /// the hash — so the two outputs together form a one-way binding.
    ///
    /// Typical operator workflow:
    ///   1. Run `sumo-tool bootstrap-token --vm-id vm9 \
    ///         --token-out vm9.token --state-out vm9-fragment.yaml`
    ///   2. Place `vm9.token` in the guest's firmware bank at
    ///      `<bank>/vhsm-bootstrap.token` (guest deletes after ENROLL).
    ///   3. Merge `vm9-fragment.yaml` into the daemon's
    ///      `<keystore>/bootstrap.yaml` under the `tokens:` map.
    ///
    /// Without --state-out, the YAML fragment is written to stdout.
    BootstrapToken {
        /// Principal name (cert subject) the token will authorise. Required
        /// for the YAML fragment.
        #[arg(long)]
        vm_id: String,

        /// Output path for the raw 32-byte token (binary). Required —
        /// without it the token would be discarded since it can't be
        ...
        /// Emit a complete standalone `bootstrap.yaml` document instead of
        /// a fragment. Use this when initialising a fresh bootstrap state
    ...
    ///
    /// The in-box ENROLL flow (handled by vhsm-ssd) is the preferred path
    /// for first-boot enrolment. This subcommand exists for:
    ///
    ///   - Pre-provisioning certs ahead of guest deployment.
    ///   - Cert rotation without going through ENROLL again.
    ///   - Test rigs that need to construct fixtures without a running daemon.
    ///
    /// The minted CWT carries the standard claim set (iss, sub, aud,
    /// exp, nbf, iat, cti, cnf) and signs as COSE_Sign1 / ES256.
    ///
    /// `aud` is hard-coded to `vhsm-ssd` to match what the daemon's
    /// validate() accepts.
    CwtMint {
        ...
        /// Principal name (cert subject; ends up in the CWT's `sub` claim).
        #[arg(long)]
        vm_id: String,

        /// Issuer string for the `iss` claim. Operator-chosen device label,
        /// e.g. `device-fleet-7`. Informational on the daemon side.
        #[arg(long, default_value = "device-vhsm-tool")]
        issuer: String,

        /// Path to the guest's identity pubkey (COSE_Key CBOR, EC2 P-256).
        /// The `cnf` claim binds this pubkey.
        #[arg(long)]
        cnf_pub: PathBuf,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        ...
        Command::BootstrapToken {
            ...
            use sumo_offboard::bootstrap;

            let token = bootstrap::generate_token();
            let sha = bootstrap::token_sha256_hex(&token);

            // Token output: write the raw bytes with restrictive
            // perms so it isn't world-readable while sitting on the
            // operator's machine. Best-effort; failures here log a
            // warning but don't abort.
            fs::write(&token_out, token)?;
            #[cfg(unix)]
            {
                use std::os::unix::fs::PermissionsExt as _;
                if let Err(e) = fs::set_permissions(&token_out, fs::Permissions::from_mode(0o600)) {
                    eprintln!(
            ...
            let yaml = if standalone {
                bootstrap::bootstrap_yaml_document(&vm_id, &sha)
            } else {
                bootstrap::bootstrap_yaml_fragment(&vm_id, &sha)
            };
            match state_out {
                Some(p) => {
                    fs::write(&p, &yaml)?;
                ...
                None => {
                    // Fragment to stdout — operator can pipe into a merge
                    // tool or copy-paste into bootstrap.yaml manually.
                    print!("{yaml}");
                }
            }
        }
        ...
        Command::CwtMint {
            ...
            // Load cnf pubkey from COSE_Key CBOR; extract raw x/y.
            let cnf_bytes = fs::read(&cnf_pub)?;
            let cnf_cose = CoseKey::from_cose_key_bytes(&cnf_bytes)?;
            let (x, y) = cnf_cose.ec2_public_xy()?;

            // Pin `iat` to the current UTC second. Caller can always
            // post-process the CWT if they need a deterministic clock
            // (e.g., for test fixtures).
            let now_unix = SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .map_err(|e| format!("system time before epoch: {e}"))?
                .as_secs();
            let validity = ValidityWindow::from_now(now_unix, lifetime_secs);

            let cwt = mint_cwt(&signer_sk, &vm_id, &issuer, &x, &y, validity)?;
            fs::write(&output, &cwt)?;
            eprintln!(
